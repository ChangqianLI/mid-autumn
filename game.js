...
const endingMusicHost = document.getElementById("endingMusicHost");
...
function resetGame(){
  ...
  if(endingMusicHost) endingMusicHost.innerHTML="";
...
// The anime's ending theme is "ひとりごつ".
// We stream it directly at result time rather than bundling copyrighted audio in the ZIP.
const ENDING_YOUTUBE_ID = "mnO0uTHjQCQ";

function playEndingMusic(){
  if(!endingMusicHost) return;

  // endGame() is triggered by the player's E/Enter interaction,
  // so the iframe is created immediately from a user gesture.
  endingMusicHost.innerHTML = `
    <iframe
      width="1"
      height="1"
      src="https://www.youtube.com/embed/${ENDING_YOUTUBE_ID}?autoplay=1&controls=0&playsinline=1&rel=0&loop=1&playlist=${ENDING_YOUTUBE_ID}"
      title="Hitorigotsu"
      frameborder="0"
